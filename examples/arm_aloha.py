...
def test_body_ids(model: mujoco.MjModel) -> None:
    ...
    assert set(model.body(name).id for name in ["left/upper_arm_link", "right/upper_arm_link"]) == {
        model.body("left/upper_arm_link").id,
        model.body("right/upper_arm_link").id,
    }
    assert set(model.body(name).id for name in ["metal_frame"]) == {
        model.body("metal_frame").id,
    }
