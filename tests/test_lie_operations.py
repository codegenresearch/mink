...
def test_adjoint(group: Type[MatrixLieGroup]):
    transform = group.sample_uniform()
...
def test_rminus(group: Type[MatrixLieGroup]):
...
def test_lminus(group: Type[MatrixLieGroup]):
    T_a = group.sample_uniform()
    T_b = group.sample_uniform()
    np.testing.assert_allclose(T_a.lminus(T_b), (T_a @ T_b.inverse()).log())


def test_rplus(group: Type[MatrixLieGroup]):
    T_a = group.sample_uniform()
    T_b = group.sample_uniform()
    T_c = T_a.inverse() @ T_b
...
def test_lplus(group: Type[MatrixLieGroup]):
    T_a = group.sample_uniform()
    T_b = group.sample_uniform()
    T_c = T_a @ T_b.inverse()
...
def test_jlog(group: Type[MatrixLieGroup]):
