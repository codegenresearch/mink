...
class TestGroupSpecificOperations(absltest.TestCase):
    ...
    def test_se3_copy(self):
        """Test SE3 copy method."""
        T = SE3.sample_uniform()
        T_copy = T.copy()
        assert_transforms_close(T, T_copy)

    def test_invalid_log_exp(self):
        """Test invalid log-exp operations."""
        with self.assertRaises(ValueError):
            SO3.exp(np.random.rand(4))  # Invalid tangent vector size

        with self.assertRaises(ValueError):
            SE3.exp(np.random.rand(6, 1))  # Invalid tangent vector shape

    def test_invalid_matrix_conversion(self):
        """Test invalid matrix conversion."""
        with self.assertRaises(ValueError):
