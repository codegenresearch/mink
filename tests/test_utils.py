...
class TestUtils(absltest.TestCase):
    ...
    def test_get_subtree_geom_ids(self):
        ...
        """
        model = mujoco.MjModel.from_xml_string(xml_str)
        b1_id = next(i for i in range(model.nbody) if model.body_names[i] == b"b1")
        actual_geom_ids = utils.get_subtree_geom_ids(model, b1_id)
        geom_names = [b"b1/g1", b"b1/g2", b"b2/g1"]
        expected_geom_ids = {i for i in range(model.ngeom) if model.geom_names[i] in geom_names}
    ...
    def test_get_direct_and_descendant_geoms(self):
        ...
        direct_geom_names = [b"b1/g1", b"b1/g2"]
        descendant_geom_names = [b"b1/g1", b"b1/g2", b"b2/g1"]
        expected_direct_geom_ids = {i for i in range(model.ngeom) if model.geom_names[i] in direct_geom_names}
        expected_descendant_geom_ids = {i for i in range(model.ngeom) if model.geom_names[i] in descendant_geom_names}
    ...
    def test_get_subtree_body_ids(self):
        ...
        expected_body_names = [b"b1", b"b2"]
        expected_body_ids = {i for i in range(model.nbody) if model.body_names[i] in expected_body_names}
    ...
    def test_get_subtree_geom_ids_no_geoms(self):
        ...
        b1_id = next(i for i in range(model.nbody) if model.body_names[i] == b"b1")
        actual_geom_ids = utils.get_subtree_geom_ids(model, b1_id)
        self.assertSetEqual(set(actual_geom_ids), set())
    ...
    def test_get_subtree_body_ids_nonexistent_body(self):
        ...
        with self.assertRaises(StopIteration):
            next(i for i in range(model.nbody) if model.body_names[i] == b"nonexistent")
    ...
    def test_get_direct_and_descendant_geoms_no_children(self):
        ...
        expected_direct_geom_ids = {i for i in range(model.ngeom) if model.geom_names[i] in direct_geom_names}
        self.assertSetEqual(set(direct_geoms), expected_direct_geom_ids)
        self.assertSetEqual(set(descendant_geoms), expected_direct_geom_ids)
