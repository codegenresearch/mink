...
class TestUtils(absltest.TestCase):
    ...
    def test_get_subtree_body_ids(self):
        ...
        # Test for a body with no children
        b7_id = model.body("b7").id
        actual_body_ids = set(utils.get_subtree_body_ids(model, b7_id))
        body_names = ["b7"]
        expected_body_ids = {model.body(g).id for g in body_names}
        self.assertSetEqual(actual_body_ids, expected_body_ids)
    ...
    def test_gravity_compensation(self):
        ...
        def apply_gravity_compensation(model, data):
            # Example implementation: apply gravity compensation to the first 6 DoFs
        ...
        apply_gravity_compensation(self.model, self.data)

        # Verify that the forces have been applied
        self.assertTrue(np.any(self.data.xfrc_applied != 0))
